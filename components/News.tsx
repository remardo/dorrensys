...
const News: React.FC<NewsProps> = ({ items }) => {
  if (!items.length) {
    return <div className="text-sm text-gray-500">Нет новостей</div>;
  }
  ...
  return (
    <div className="animate-fade-in space-y-8">
      <div className="flex justify-between items-center">
        ...
        <button className="text-xs uppercase tracking-wider text-dorren-dark hover:text-dorren-blue flex items-center">
          Архив <ArrowRight size={14} className="ml-1" />
        </button>
      </div>

      <div className="grid lg:grid-cols-2 gap-8">
        <div className="lg:col-span-2 relative overflow-hidden bg-black group cursor-pointer">
          <img
            src={first.image}
            alt={first.title}
      ...
      <section className="bg-white border border-gray-100 p-6">
        ...
        <div className="space-y-3">
          {items.slice(0, 4).map((text) => (
            <div key={text.id} className="flex justify-between items-center border-b border-gray-50 pb-3">
