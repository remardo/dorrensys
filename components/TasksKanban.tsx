...
import { LayoutPanelLeft, Rows, Plus, Trash } from 'lucide-react';
import TaskCard from './tasks/TaskCard';
import NewTaskModal from './tasks/NewTaskModal';
import PdfStorageList from './tasks/PdfStorageList';
...
const TasksKanban: React.FC = () => {
  const { tasks, users, docs, loading, error, assignTask, moveTask, addTask, deleteTask } = useTasksData();
  ...
  const filtered = useMemo(() => {
    return tasksWithAssignee.filter((t) => {
      const a =
        ...
        (!t.assignee && filters.assignee === 'unassigned') ||
        t.assigneeEmail === filters.assignee ||
  ...
  const renderColumn = (status: Task['status']) => (
    <div className="flex-1 min-w-[320px]" onDragOver={(e) => e.preventDefault()} onDrop={() => dragging && handleMove(dragging, status)}>
      ...
      <div className="space-y-3">
        ...
        <button className="w-full py-3 border border-dashed border-gray-300 text-gray-400 text-xs uppercase tracking-wider hover:border-dorren-blue hover:text-dorren-blue hover:bg-dorren-blue/5 transition-all flex items-center justify-center rounded-sm" onClick={() => setNewModal(true)}>
    ...
    <div className="animate-fade-in h-full flex flex-col">
      <div className="flex justify-between items-center mb-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">Данные подтягиваются из Convex. Изменения синхронизируются при каждом действии.</p>
        </div>
      ...
      {error && <div className="mb-4 text-sm text-red-600 border border-red-100 bg-red-50 px-3 py-2">{error}</div>}
            ...
            <tbody>
              {filtered.map((task) => (
                <tr key={task.id} className="border-b border-gray-50 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <select className="text-xs border border-gray-200 px-2 py-1" value={task.status} onChange={(e) => handleMove(task.id, e.target.value as Task['status'])}>
      ...
      <PdfStorageList docs={docs} />

      {newModal && <NewTaskModal users={users} onClose={() => setNewModal(false)} onSave={handleAddTask} />}
        ...
        <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-3xl shadow-2xl border border-gray-100 p-6">
            ...
            <div className="grid md:grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <div className="text-xs text-gray-500 uppercase mb-1">Создано</div>
